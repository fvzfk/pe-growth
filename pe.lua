-- ============================================================
-- HackerAI | Prior Extinction Growth Automation v4.1
-- Opiumware Stable | One-Liner Ready
-- ============================================================
...
local function safeHttpGet(url)
    ...
    warn("[HackerAI] Failed to fetch: " .. url)
    return nil
end
...
local function loadExternal(url)
    ...
    if not src then
        error("[HackerAI] Critical load failed: " .. url)
    end
    return loadstring(src)()
end

-- Load compatibility layer (optional, ignore if fails)
pcall(function() loadstring(safeHttpGet("https://raw.githubusercontent.com/luau/Executor-API-Docs/master/script/compatibility_layer.lua"))() end)

-- Load Linoria UI
local repo = "https://raw.githubusercontent.com/wally-rblx/LinoriaLib/main/"
local Library = loadExternal(repo .. "Library.lua")
local ThemeManager = loadExternal(repo .. "addons/ThemeManager.lua")
local SaveManager = loadExternal(repo .. "addons/SaveManager.lua")
...
local function sendWebhook(isEdit)
    local embed = {
        ...
        ["footer"] = { ["text"] = "HackerAI Growth Automation v4.1 | Updates every 15s" },
...
local function updateWebhook()
    if tick() - lastWebhookUpdate < 15 then return end
    lastWebhookUpdate = tick()
    sendWebhook(CONFIG.WebhookMessageId ~= nil)
end
...
local function detectDiet()
    ...
    local knownDinos = {
        ...
        ["pteranodon"] = "Carnivore", ["pterodactyl"] = "Carnivore",
        ...
        ["kentrosaurus"] = "Herbivore", ["styracosaurus"] = "Herbivore",
        ...
        ["majangasuchus"] = "Carnivore", ["protoceratops"] = "Herbivore", ["psittacosaurus"] = "Herbivore",
        ["chasmosaurus"] = "Herbivore", ["centrosaurus"] = "Herbivore", ["sinoceratops"] = "Herbivore",
        ["tarchia"] = "Herbivore", ["saichania"] = "Herbivore", ["pinacosaurus"] = "Herbivore",
        ["austroraptor"] = "Carnivore", ["buitreraptor"] = "Carnivore", ["pyroraptor"] = "Carnivore",
        ["achillobator"] = "Carnivore", ["dakotaraptor"] = "Carnivore", ["australovenator"] = "Carnivore",
        ["mapusaurus"] = "Carnivore", ["meraxes"] = "Carnivore", ["titanosaurus"] = "Herbivore",
        ["argentinosaurus"] = "Herbivore", ["puertasaurus"] = "Herbivore", ["dreadnoughtus"] = "Herbivore",
        ["ampe"] = "Herbivore", ["hypsilophodon"] = "Herbivore", ["tenontosaurus"] = "Herbivore",
        ["dryosaurus"] = "Herbivore", ["camarasaurus"] = "Herbivore", ["shunosaurus"] = "Herbivore",
        ["mamenchisaurus"] = "Herbivore", ["rhabdodon"] = "Herbivore",
    }

    if char then
        -- Check attributes
        for _, attrName in ipairs({"Diet", "diet", "FoodType", "foodType"}) do
            local attr = char:GetAttribute(attrName)
            if attr then
                local a = tostring(attr):lower()
                if a:find("carn") then return "Carnivore", a end
                if a:find("herb") then return "Herbivore", a end
                if a:find("omni") or a:find("pisci") then return "Omnivore", a end
            end
        end
        ...
        for _, v in ipairs(char:GetDescendants()) do
            if v:IsA("StringValue") or v:IsA("NumberValue") then
                ...
                if n:find("diet") or n:find("food") or n:find("eats") then
                    if val:find("carn") then return "Carnivore", v.Value end
                    if val:find("herb") then return "Herbivore", v.Value end
                    if val:find("omni") or val:find("pisci") then return "Omnivore", v.Value end
                end
            end
        end
    ...
    -- ReplicatedStorage data scan
    local searchFolders = {"Dinosaurs", "DinoData", "Species", "Creatures", "Stats", "Data"}
    for _, folderName in ipairs(searchFolders) do
        local folder = ReplicatedStorage:FindFirstChild(folderName)
        if folder then
            for _, v in ipairs(folder:GetDescendants()) do
                local n = v.Name:lower()
                if n:find("diet") or n:find("food") then
                    local val = ""
                    if v:IsA("StringValue") then val = v.Value:lower() elseif v:IsA("Attribute") then val = tostring(v):lower() end
    ...
    -- PlayerGui / wellbeing menu scan
    local function scanGui(container)
        if not container then return nil end
        for _, v in ipairs(container:GetDescendants()) do
            if v:IsA("TextLabel") or v:IsA("TextButton") then
                local t = v.Text:lower()
                if t:find("carnivore") then return "Carnivore", v.Text end
                if t:find("herbivore") then return "Herbivore", v.Text end
                if t:find("omnivore") or t:find("piscivore") then return "Omnivore", v.Text end
                for dinoName, dinoDiet in pairs(knownDinos) do
                    if t:find(dinoName) then return dinoDiet, dinoName end
                end
            end
        end
        return nil
    ...
    local guiResult = scanGui(LocalPlayer:FindFirstChild("PlayerGui")) or scanGui(CoreGui)
    if guiResult then return guiResult end
    ...
    if playerGui then
        for _, v in ipairs(playerGui:GetDescendants()) do
            if v:IsA("TextLabel") then
                local t = v.Text
                if t:find("saurus") or t:find("raptor") or t:find("don") or t:find("tops") or t:find("chus") or t:find("suchus") then
                    return t
                end
            end
        end
...
local function detectGrowthStage()
        ...
        for _, v in ipairs(container:GetDescendants()) do
            if v:IsA("TextLabel") or v:IsA("TextButton") then
                for _, stage in ipairs(stages) do
                    if v.Text:lower():find(stage:lower()) then
                        return stage
                    end
                end
            end
    ...
    return scan(LocalPlayer:FindFirstChild("PlayerGui")) or scan(CoreGui) or CONFIG.GrowthStage
end
...
local function detectGrowthProgress()
    ...
    return scan(LocalPlayer:FindFirstChild("PlayerGui")) or scan(CoreGui) or CONFIG.GrowthProgress
end
...
local function checkModerators()
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then
            ...
            if n:find("mod") or n:find("admin") or n:find("staff") or n:find("cm") or n:find("gm") or n:find("dev") then
...
local function checkServerRestart()
    for _, v in ipairs(CoreGui:GetDescendants()) do
        if v:IsA("TextLabel") then
            local t = v.Text:lower()
            if t:find("restart") or t:find("shutdown") or t:find("server will") or t:find("updating") then
                return true
            end
        end
    end
...
local function serverHop()
    ...
    pcall(function()
        local success, servers = pcall(function()
            return HttpService:JSONDecode(game:HttpGet("https://games.roblox.com/v1/games/" .. placeId .. "/servers/Public?limit=100", true))
        end)
...
local function enterSafeMode(reason)
    if SafeModeActive or not CONFIG.SafeModeEnabled then return end
    SafeModeActive = true
...
G:AddLabel("MASTER TOGGLE — Auto-detects diet, full pipeline")
G:AddToggle("AutoGrowth", { Text = "▶ Auto Growth (Master Toggle)", Default = false })
G:AddDropdown("StopAtStage", { Values = { "Don't Stop", "Juvenile", "Teen", "Sub-Adult", "Adult", "Elder", "Custom %" }, Default = 1, Text = "Stop Growing At" })
G:AddSlider("CustomStopPercent", { Text = "Custom Stop %", Default = 100, Min = 1, Max = 100, Rounding = 1, Visible = false })
G:AddSlider("ScanRange", { Text = "Food Scan Range", Default = 300, Min = 50, Max = 1000, Rounding = 1 })
G:AddSlider("EatDuration", { Text = "Eat Duration (seconds)", Default = 8, Min = 2, Max = 30, Rounding = 1 })
G:AddSlider("CircleDuration", { Text = "Circle Duration (seconds)", Default = 20, Min = 5, Max = 60, Rounding = 1 })
G:AddToggle("InfiniteWater", { Text = "Auto Drink (Infinite Water)", Default = true })
G:AddSlider("DrinkInterval", { Text = "Drink Interval (seconds)", Default = 30, Min = 10, Max = 120, Rounding = 1 })
...
Options.StopAtStage:OnChanged(function() CONFIG.StopAtStage = Options.StopAtStage.Value Options.CustomStopPercent:SetVisible(Options.StopAtStage.Value == "Custom %") end)
Options.CustomStopPercent:OnChanged(function() CONFIG.CustomStopPercent = Options.CustomStopPercent.Value end)
...
Toggles.InfiniteWater:OnChanged(function() CONFIG.InfiniteWater = Toggles.InfiniteWater.Value end)

-- Safety Tab
local S = Tabs.Safety:AddLeftGroupbox("Damage & Safety")
S:AddToggle("SafeModeEnabled", { Text = "Emergency Safe Mode", Default = true })
S:AddToggle("NoFallDamage", { Text = "No Fall Damage", Default = true })
S:AddToggle("AvoidPlayers", { Text = "Avoid Players Near Food", Default = true })
S:AddSlider("DangerRange", { Text = "Danger Detection Range", Default = 300, Min = 50, Max = 500, Rounding = 1 })

local S2 = Tabs.Safety:AddRightGroupbox("Server Safety")
S2:AddToggle("ServerHopMods", { Text = "Server Hop on Moderators", Default = true })
S2:AddToggle("ServerHopRestart", { Text = "Server Hop on Restart", Default = true })
S2:AddToggle("AutoSave", { Text = "Auto Save Dino", Default = true })
S2:AddSlider("SaveInterval", { Text = "Save Interval (seconds)", Default = 90, Min = 30, Max = 600, Rounding = 1 })
...
Dbg:AddButton("Re-detect Diet & Dino", function()
    ...
    CONFIG.DietType = diet ~= "Unknown" and diet or CONFIG.DietType
    CONFIG.DinoName = detectDinoName()
    Library:Notify("Diet: " .. CONFIG.DietType .. " | Dino: " .. CONFIG.DinoName)
end)
Dbg:AddButton("Force Save Now", function() lastSaveTime = 0 doSave() end)
Dbg:AddButton("Emergency Sky Teleport", function() teleportTo(findSafeSkyPosition()) end)
Dbg:AddButton("Find Safe Ground (2000 studs)", function() teleportTo(findSafeGroundPosition()) end)
Dbg:AddButton("Test Webhook", function() sendWebhook(false) end)
...
local function findAllFood(range)
    ...
    local carniTerms = { "corpse", "body", "dead", "remains", "carcass", "bone", "skeleton", "meat", "ragdoll", "food_carn", "carrion", "flesh", "kill", "prey" }
    local herbiTerms = { "plant", "bush", "berry", "flower", "grass", "leaf", "fern", "vegetation", "tree_food", "fruit", "food_herb", "foliage", "shoot", "reed", "moss", "algae", "seed", "nut", "root" }
        ...
        if v:IsA("BasePart") or v:IsA("Model") then
            ...
            if matched then
                ...
                if vPos then
                    ...
                    if dist <= range then
                        table.insert(results, { Instance = v, Position = vPos, Distance = dist, Type = "Food" })
                    end
                end
            end
        end
    ...
    -- Player corpses (dead dino characters)
            ...
            if hum and hum.Health <= 0 then
                ...
                if ref then
                    ...
                    if dist <= range then
                        ...
                        if not isOurs then
                            table.insert(results, { Instance = v, Position = ref.Position, Distance = dist, Type = "Player Corpse" })
                        end
                    end
                end
            end
            ...
            if hum then
                ...
                if isDead then
                    ...
                    if ref then
                        local dist = (ref.Position - pos).Magnitude
                        if dist <= range then
                            table.insert(results, { Instance = player.Character, Position = ref.Position, Distance = dist, Type = "Player Corpse" })
                        end
                    end
                end
            end
...
local function startGrowthEngine()
    ...
    Library:Notify("🚀 Growth started | Diet: " .. CONFIG.DietType .. " | Dino: " .. CONFIG.DinoName)
    sendWebhook(false)
    task.wait(1)
    ...
    GrowthLoop = RunService.RenderStepped:Connect(function()
            ...
            if #foodList > 0 then
                if CONFIG.AvoidPlayers then
                    for _, food in ipairs(foodList) do
                        local safe, _ = isPositionSafe(food.Position, CONFIG.DangerRange)
                        if safe then foodTarget = food break end
                    end
                end
                if not foodTarget then foodTarget = foodList[1] end
                state = "move_to_food"
            else
        ...
        elseif state == "move_to_food" then
            if not foodTarget or not foodTarget.Instance or not foodTarget.Instance.Parent then
                state = "search_food"
                return
            end
...
UI:AddButton("Unload Script", function() cleanup() Library:Unload() end)
UI:AddLabel("Menu bind"):AddKeyPicker("MenuKeybind", { Default = "End", NoUI = true, Text = "Menu keybind" })
...
Library:Notify("✅ HackerAI Growth v4.1 loaded | Prior Extinction")
...
task.spawn(function()
    ...
    local name = detectDinoName()
    CONFIG.DietType = diet ~= "Unknown" and diet or CONFIG.DietType
    CONFIG.DinoName = name ~= "Unknown" and name or CONFIG.DinoName
    CONFIG.GrowthStage = detectGrowthStage()
    CONFIG.GrowthProgress = detectGrowthProgress()
    print("[HackerAI] Detected: Diet=" .. CONFIG.DietType .. " Dino=" .. CONFIG.DinoName .. " Stage=" .. CONFIG.GrowthStage)
end)

print("=== HackerAI Prior Extinction Growth v4.1 ===")
print("Player: " .. LocalPlayer.Name)
print("Place: " .. game.PlaceId .. " | Job: " .. game.JobId)
print("Webhook: " .. (WEBHOOK_URL ~= "" and "Configured" or "NOT SET"))
